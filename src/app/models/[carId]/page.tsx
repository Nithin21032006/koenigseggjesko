...
export default function ModelShowcasePage({ params }: { params: Promise<{ carId: string }> }) {
  const { carId } = use(params);
  const { setSelectedCarId } = useConfigurator();
  ...
  // When visiting a car's showcase, automatically set it as the active car in context
  useEffect(() => {
    setSelectedCarId(car.id);
  }, [car.id, setSelectedCarId]);
  ...
  return (
    <main className="bg-jesko-black min-h-screen">
      ...
      <section ref={containerRef} className="h-[600vh] relative">
        <div className="sticky top-0 h-screen w-full overflow-hidden bg-jesko-black">
          <CarScrollCanvas 
            ...
            imageFolderPath={car.sequenceFolder} 
            prefix={car.sequencePrefix}
      ...
      <footer className="border-t border-carbon-gray py-8 text-center text-gray-500 font-rajdhani text-sm tracking-widest bg-jesko-black relative z-20">
        &copy; {new Date().getFullYear()} KOENIGSEGG DEALERSHIP. CONCEPT DESIGN.
      </footer>
    </main>
  );
}

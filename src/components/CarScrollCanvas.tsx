...
export default function CarScrollCanvas({
  ...
  useEffect(() => {
    ...
    const preloadImages = () => {
      for (let i = 1; i <= totalFrames; i++) {
        const img = new Image();
        const numStr = prefix ? String(i).padStart(3, '0') : String(i);
        img.src = `${imageFolderPath}/${prefix}${numStr}.jpg`;
    ...
    preloadImages();
  }, [totalFrames, imageFolderPath, prefix, onProgress]);
  ...
  return (
    <div className="absolute inset-0 z-0 bg-jesko-black">
      <canvas
        ref={canvasRef}
